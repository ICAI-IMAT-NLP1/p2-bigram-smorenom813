...
def bigrams_count_to_probabilities(
    ...
    # TODO
    print("Dimensiones de bigram_counts:", bigram_counts.size())

    for i in range(bigram_counts.size(0)):
        print(i)
        ocurrencies = bigram_counts[i,:].sum()
        bigram_counts[i,:] = (bigram_counts[i,:]+smooth_factor)/(ocurrencies+ smooth_factor*bigram_counts.size(1))
...
def sample_next_character(
    ...
    # TODO
    next_char_index: int = torch.argmax(current_probs).item()
...
def calculate_log_likelihood(
    ...
    # TODO
    log_likelihood: torch.tensor = torch.tensor(0.0, dtype=torch.float64)
    bigram_probabilities_log: torch.Tensor = torch.log(bigram_probabilities)

    # Iterate through bigrams in the word and accumulate their log probabilities
    for i in range(len(processed_word)-1):
        x = char_to_index[processed_word[i]]
        y = char_to_index[processed_word[i+1]]
        log_likelihood+= bigram_probabilities_log[x,y]
